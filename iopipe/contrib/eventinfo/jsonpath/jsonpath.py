...
class Child(JSONPath):
    ...
    def find(self, datum):
        ...
        return [
            submatch for subdata in self.left.find(datum) if not isinstance(subdata, AutoIdForDatum)
    ...
    def find(self, datum):
        ...
        def match_recursively(datum):
            ...
            if isinstance(datum.value, list):
                recursive_matches = [
                    submatch for i in range(0, len(datum.value))
                ...
                recursive_matches = [
                    submatch for field in datum.value.keys() for submatch in match_recursively(
...
class Fields(JSONPath):
        ...
        if field == auto_id_field:
            ...
            try:
                field_value = datum.value[
                    field]  # Do NOT use `val.get(field)` since that confuses None as a value and None due to `get`
    ...
    def find(self, datum):
        ...
        return [
            field_datum for field_datum in [self.get_field_datum(datum, field) for field in self.reified_fields(datum)]
...
class Slice(JSONPath):
    ...
    def __str__(self):
        if self.start is None and self.end is None and self.step is None:
            ...
            return '[%s%s%s]' % (self.start or '', ':%d' % self.end if self.end else '', ':%d' % self.step
                                 if self.step else '')
