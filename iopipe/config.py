...
def set_config(**config):
    ...
    if '.' in str(config['network_timeout']):
        warnings.warn("IOpipe's 'network_timeout' is now in milliseconds, expressed as an integer")
    ...
    if '.' in str(config['timeout_window']):
        warnings.warn("IOpipe's 'timeout_window' is now in milliseconds, expressed as an integer")
